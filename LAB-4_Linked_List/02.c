// Write a menu driven program to perform the following operations in a double linked list (using self-referencing structure) by using suitable user defined functions for each case.
...
node *delete_specific(node *start)
{
    ...
    int choice, pos = 0, count = 0, value, len = lenLL(start);
